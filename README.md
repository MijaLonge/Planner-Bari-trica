...
<head>
    ...
    <title>Planner Bariátrico Interativo</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <!-- Chosen Palette: Warm Neutral Harmony -->
    <!-- Application Structure Plan: A SPA com duas seções principais (abas): "Pré-Cirurgia (Despertar)" e "Pós-Operatório (Florescer)", controladas por JS. A estrutura foi escolhida para separar claramente as duas fases distintas da jornada. "Despertar" foca em planejamento e checklists (formulários e tabelas dinâmicas) e feedback emocional via LLM. "Florescer" é um dashboard focado em data-logging (formulários de entrada), visualização de dados (gráficos de linha dinâmicos) e análise de correlação dos logs via LLM. O fluxo do usuário é: 1. Navegar entre as fases. 2. Registrar dados. 3. Usar os botões LLM para obter insights personalizados. 4. Visualizar a evolução. -->
    <!-- Visualization & Content Choices: [Info: Controle de Peso/Medidas -> Goal: Rastrear mudança -> Viz: 2 Gráficos de Linha (Chart.js/Canvas) -> Interaction: Formulário adiciona dados ao array JS, que atualiza os gráficos -> Justification: Melhor método para tendências de tempo. | Info: Consultas, Tarefas, Diários (Alimentar/Emocional) -> Goal: Organizar/Registrar -> Viz: Tabelas HTML -> Interaction: Formulários adicionam novas linhas (<tr>) às tabelas via JS DOM -> Justification: Transforma o planner estático em uma ferramenta de log interativa. | Info: Reflexões, Metas -> Goal: Informar/Registrar/Gerar Feedback -> Viz: <textarea> e Bloco de Resultados LLM -> Interaction: Botão LLM coleta dados de reflexão e gera sugestões de preparo. | Info: Logs Alimentar/Emocional -> Goal: Analisar correlações -> Viz: Bloco de Resultados LLM -> Interaction: Botão LLM coleta logs para análise de tendências e sugestões de foco.] -->
...
<body class="antialiased">
    ...
    <div class="container mx-auto p-4 md:p-8 max-w-7xl">
        ...
        <header class="text-center mb-8">
            <h1 class="text-4xl md:text-5xl font-bold mb-2">Planner Bariátrico</h1>
        ...
        <main>
            <!-- ======================= -->
            <!-- PÁGINA 1: DESPERTAR (PRÉ) -->
            <!-- ======================= -->
            <section id="page-despertar" class="page-content space-y-6">
                <div class="card p-6 rounded-lg shadow-md">
                    <h2 class="text-3xl font-bold mb-3">Fase 1: Pré-Cirurgia (Despertar)</h2>
                    <p class="text-lg text-gray-700">
                        Esta é a sua fase de preparação. O foco aqui é organização, autoconhecimento e planejamento emocional e logístico para o grande dia. Use as ferramentas abaixo para registrar suas reflexões, dúvidas e organizar seus compromissos.
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="card p-6 rounded-lg shadow-md space-y-4">
                        <h3 class="text-2xl font-semibold">1. Autoconhecimento e Reflexões</h3>
                        <div>
                            <label for="reflexao-porque" class="block text-md font-medium text-gray-700 mb-1">Por que a Cirurgia?</label>
                            <textarea id="reflexao-porque" rows="3" class="form-textarea w-full rounded-md p-2" placeholder="Qual é a minha motivação mais profunda? O que espero que mude?"></textarea>
                        </div>
                        <div>
                            <label for="reflexao-gatilhos" class="block text-md font-medium text-gray-700 mb-1">Gatilhos Emocionais</label>
                            <textarea id="reflexao-gatilhos" rows="3" class="form-textarea w-full rounded-md p-2" placeholder="Quais situações me levam a comer em excesso hoje?"></textarea>
                        </div>
                        <div>
                            <label for="reflexao-compromisso" class="block text-md font-medium text-gray-700 mb-1">Meu Compromisso</label>
                            <textarea id="reflexao-compromisso" rows="3" class="form-textarea w-full rounded-md p-2" placeholder="Qual é o meu nível de preparo com as mudanças permanentes?"></textarea>
                        </div>
                        ...
                        <button id="btn-sugestoes-preparo" class="btn-gemini w-full rounded-md p-2 font-semibold flex items-center justify-center space-x-2">
                            ...
                            <span id="text-despertar">Gerar Sugestões de Preparo Emocional ✨</span>
                        </button>
                    ...
                    <div class="card p-6 rounded-lg shadow-md space-y-4">
                        ...
                        <div>
                            <label for="expectativa-aprendizado" class="block text-md font-medium text-gray-700 mb-1">O que Aprendi até Agora</label>
                            <textarea id="expectativa-aprendizado" rows="2" class="form-textarea w-full rounded-md p-2" placeholder="Conhecimentos sobre alimentação, metabolismo..."></textarea>
                        </div>
                        <div>
                            <label for="expectativa-duvidas" class="block text-md font-medium text-gray-700 mb-1">Espaço para Dúvidas (para a equipe)</label>
                    ...
                    <div class="card p-6 rounded-lg shadow-md lg:col-span-1">
                        ...
                        <form id="form-consulta" class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            ...
                            <input id="consulta-obs" type="text" class="form-input rounded-md p-2 sm:col-span-2" placeholder="Observações">
                    ...
                    <div class="card p-6 rounded-lg shadow-md lg:col-span-1">
                        ...
                        <form id="form-tarefa" class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            <input id="tarefa-area" type="text" class="form-input rounded-md p-2" placeholder="Área (Ex: Casa, Logística)" required>
            ...
            <!-- ========================= -->
            <!-- PÁGINA 2: FLORESCER (PÓS) -->
            <!-- ========================= -->
            <section id="page-florescer" class="page-content hidden space-y-6">
                <div class="card p-6 rounded-lg shadow-md">
                    <h2 class="text-3xl font-bold mb-3">Fase 2: Pós-Operatório (Florescer)</h2>
                    <p class="text-lg text-gray-700">
                        A cirurgia passou e o verdadeiro processo começa. Esta seção é seu dashboard para construir hábitos, acompanhar sua evolução e se reconectar com você. Registre seu peso, medidas e diários para visualizar seu progresso nos gráficos.
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="card p-6 rounded-lg shadow-md">
                            <h3 class="text-2xl font-semibold mb-4">Adicionar Medição</h3>
                            <form id="form-medidas" class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    <input id="medida-braco" type="number" class="form-input w-full rounded-md p-2" placeholder="Braço (cm)">
                                </div>
                                <button type="submit" class="btn-primary w-full rounded-md p-2 font-semibold">Salvar Medição e Atualizar Gráficos</button>
                            </form>
                        </div>

                        <!-- Card: Diário Alimentar -->
                        <div class="card p-6 rounded-lg shadow-md">
                            <h3 class="text-2xl font-semibold mb-4">3. Diário Alimentar</h3>
                            <form id="form-diario-alimentar" class="space-y-3">
                                ...
                                <input id="alim-refeicao" type="text" class="form-input w-full rounded-md p-2" placeholder="Refeição (Ex: Almoço)" required>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    <select id="alim-tolerancia" class="form-select w-full rounded-md p-2" required>
                                        <option value="Ótima">Tolerância: Ótima</option>
                                        <option value="Média">Tolerância: Média</option>
                                        <option value="Ruim">Tolerância: Ruim</option>
                                    </select>
                                </div>
                                <button type="submit" class="btn-primary w-full rounded-md p-2 font-semibold">Registrar Refeição</button>
                            </form>
                        </div>

                        <!-- Card: Geração de Receitas por Fase -->
                        <div class="card p-6 rounded-lg shadow-md">
                            <h3 class="text-2xl font-semibold mb-4">6. Sugestão de Receitas ✨</h3>
                            <p class="text-sm text-gray-600 mb-3">Escolha sua fase atual e gere ideias de receitas focadas em proteína e tolerância.</p>
                            <div class="space-y-3">
                                <select id="recipe-fase" class="form-select w-full rounded-md p-2" required>
                                    <option value="">Selecione a Fase Atual</option>
                                    <option value="Líquida">Fase Líquida (Ex: Caldos, Sucos coados)</option>
                                    <option value="Cremosa">Fase Cremosa (Ex: Purês, Sopas cremosas)</option>
                                    ...
                                    <option value="Sólida">Fase Sólida (Ex: Alimentação regular balanceada)</option>
                                </select>
                                <button id="btn-gerar-receita" class="btn-gemini w-full rounded-md p-2 font-semibold flex items-center justify-center space-x-2">
                                    ...
                                    <span id="text-receita">Gerar Ideias de Receitas por Fase ✨</span>
                                </button>
                            </div>
                    ...
                    <!-- Coluna 2: GRÁFICOS E VISUALIZAÇÃO -->
                    <div class="lg:col-span-2 space-y-6">
                        <!-- Card: Gráfico de Peso -->
                        <div class="card p-6 rounded-lg shadow-md">
                            <h3 class="text-2xl font-semibold mb-4 text-center">2. Evolução do Peso (kg)</h3>
                            <div class="chart-container">
                                <canvas id="pesoChart"></canvas>
                            </div>
                        </div>

                        <!-- Card: Gráfico de Medidas -->
                        <div class="card p-6 rounded-lg shadow-md">
                            <h3 class="text-2xl font-semibold mb-4 text-center">2. Evolução das Medidas (cm)</h3>
                            <div class="chart-container">
                    ...
                    <div class="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="card p-6 rounded-lg shadow-md">
                            <h3 class="text-2xl font-semibold mb-4">4. Diário Emocional</h3>
                            <form id="form-diario-emocional" class="space-y-3">
                                <input id="emo-data" type="date" class="form-input w-full rounded-md p-2" required>
                                <div>
                                    <label for="emo-humor" class="block text-sm font-medium text-gray-700">Humor (1=Ruim, 5=Ótimo)</label>
                                ...
                                <textarea id="emo-lidei" rows="2" class="form-textarea w-full rounded-md p-2" placeholder="Como lidei com a emoção?"></textarea>
                                <textarea id="emo-gratidao" rows="2" class="form-textarea w-full rounded-md p-2" placeholder="Gratidão e Metas para amanhã"></textarea>
                                <button type="submit" class="btn-primary w-full rounded-md p-2 font-semibold">Registrar Emoção</button>
                            </form>
                        </div>
                        
                        <!-- Card: Metas e Bem-Estar -->
                        <div class="card p-6 rounded-lg shadow-md">
                            <h3 class="text-2xl font-semibold mb-4">5. Metas, Gratidão e Bem-Estar</h3>
                            <div class="space-y-4">
                                <div>
                                    <label for="metas-nao-escala" class="block text-md font-medium text-gray-700 mb-1">Minhas Metas de "Não-Escala"</label>
                                ...
                                <h4 class="text-lg font-medium text-gray-700">Checklist de Bem-Estar Diário</h4>
                                <div class="space-y-2">
                                    <label class="flex items-center">
                                        <input type="checkbox" class="h-5 w-5 text-[var(--accent)] rounded border-gray-300 focus:ring-[var(--accent)]">
                                        <span class="ml-2 text-gray-700">30 min de exercício/movimento</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" class="h-5 w-5 text-[var(--accent)] rounded border-gray-300 focus:ring-[var(--accent)]">
                                    ...
                                    <label class="flex items-center">
                                        <input type="checkbox" class="h-5 w-5 text-[var(--accent)] rounded border-gray-300 focus:ring-[var(--accent)]">
                                        <span class="ml-2 text-gray-700">Meditação / Momento de Calma</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" class="h-5 w-5 text-[var(--accent)] rounded border-gray-300 focus:ring-[var(--accent)]">
                                ...
                                <div class="mt-4 pt-4 border-t border-gray-200">
                                    <h4 class="text-lg font-semibold text-[var(--gemini-color)] mb-2">Motivação por Voz ✨</h4>
                                    <p class="text-sm text-gray-600 mb-3">Ouça seu lema atual em voz alta para reforçar sua meta.</p>
                        ...
                        <div class="card p-6 rounded-lg shadow-md md:col-span-2">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <h4 class="text-lg font-medium text-gray-700 mb-2">Últimas Refeições</h4>
                                    <div class="overflow-x-auto max-h-48 overflow-y-auto">
                                        <table class="w-full min-w-[400px]">
                                            <thead class="border-b-2 border-gray-200 text-left text-sm text-gray-600">
                                                <tr><th class="p-1">Data</th><th class="p-1">Refeição</th><th class="p-1">Alimento</th><th class="p-1">Tolerância</th></tr>
                                            </thead>
                                ...
                                <div>
                                    <h4 class="text-lg font-medium text-gray-700 mb-2">Últimas Emoções</h4>
                                    <div class="overflow-x-auto max-h-48 overflow-y-auto">
                                        <table class="w-full min-w-[400px]">
                                            <thead class="border-b-2 border-gray-200 text-left text-sm text-gray-600">
                                ...
                                <button id="btn-analisar-logs" class="btn-gemini w-full rounded-md p-2 font-semibold flex items-center justify-center space-x-2">
                                    ...
                                    <span id="text-florescer">Analisar Logs e Sugerir Foco ✨</span>
                                </button>

                                <div id="resultado-florescer" class="mt-4 p-3 bg-[var(--gemini-light)] rounded-md border border-[var(--gemini-color)] hidden">
                                    <h4 class="font-semibold text-[var(--gemini-color)] mb-2">Análise de Tendência e Foco Sugerido:</h4>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function callGeminiApi(prompt, systemInstruction) {
                ...
                return "Desculpe, a API não pôde gerar uma resposta após várias tentativas.";
            }

            // --- LÓGICA DO TTS: LER LEMA ---
            ...
            btnLerLema.addEventListener('click', async () => {
                ...
                try {
                    ...
                    alert("Erro ao processar a síntese de voz. Verifique o console para detalhes.");
                } finally {
            ...
            btnSugestoesPreparo.addEventListener('click', async () => {
                ...
                if (!porque && !medo) {
                    respostaDespertar.textContent = "Por favor, preencha pelo menos as seções 'Por que a Cirurgia?' ou 'Meu Maior Medo' para que eu possa gerar sugestões focadas.";
                    resultadoDespertar.classList.remove('hidden');
                    return;
                }

                const prompt = `Analise as seguintes reflexões e medos pré-bariátrica e gere uma resposta de coach motivacional (apenas texto) com 3 a 5 passos práticos e positivos de preparo emocional focado nos pontos levantados. Use bullet points. 
                - Motivação (Por que a Cirurgia?): ${porque}
                - Desafios (Gatilhos Emocionais): ${gatilhos}
                - Compromisso: ${compromisso}
                - Expectativa Principal: ${expectativa}
                - Maior Medo: ${medo}`;

                const systemInstruction = "Você é um conselheiro bariátrico experiente. Sua função é analisar as reflexões do paciente e fornecer um feedback empático, motivacional e com sugestões práticas e realistas para a preparação mental e emocional para a cirurgia e o pós-operatório.";
                
                textDespertar.classList.add('hidden');
                loadingDespertar.style.display = 'block';
                resultadoDespertar.classList.remove('hidden');
                respostaDespertar.textContent = "Analisando suas reflexões... isso pode levar alguns segundos.";
            ...
            // --- LÓGICA DO FLORESCER (PÓS-OP) - ANÁLISE DE LOGS ---
            ...
            btnAnalisarLogs.addEventListener('click', async () => {
                ...
                if (alimentares.length === 0 && emocionais.length === 0) {
                    respostaFlorescer.textContent = "Por favor, registre dados no Diário Alimentar e no Diário Emocional para que eu possa fazer uma análise.";
                    resultadoFlorescer.classList.remove('hidden');
                    return;
                }

                const alimentaresText = alimentares.map(a => 
                    `[${a.data}, Fase: ${a.fase}, Alimento: ${a.alimento}, Tolerância: ${a.tolerancia}]`
                ).join('\n');
                ...
                const prompt = `Analise os logs de bem-estar pós-bariátrica e forneça um breve resumo de tendência e uma sugestão de FOCO para a próxima semana. A análise deve correlacionar a tolerância alimentar e as emoções registradas. Formate a resposta com um título em negrito e 3 bullet points.
                ...
                const systemInstruction = "Você é um analista de tendências de bem-estar pós-bariátrica. Seu objetivo é identificar padrões (ex: 'Baixa tolerância correlacionada com ansiedade') e sugerir um único FOCO prático e motivacional (ex: 'Focar em técnicas de mastigação', 'Aumentar a atenção plena nas refeições', 'Explorar exercícios de respiração para ansiedade'). Mantenha o tom profissional e encorajador.";

                textFlorescer.classList.add('hidden');
                loadingFlorescer.style.display = 'block';
                resultadoFlorescer.classList.remove('hidden');
                respostaFlorescer.textContent = "Analisando seus logs de evolução... isso pode levar alguns segundos.";

                try {
                    const responseText = await callGeminiApi(prompt, systemInstruction);
                    respostaFlorescer.textContent = responseText;
                } catch (error) {
                    respostaFlorescer.textContent = "Ocorreu um erro ao conectar com o assistente de análise. Tente novamente mais tarde.";
                } finally {
            ...
            // --- LÓGICA DO FLORESCER (PÓS-OP) - RECEITA ---
            ...
            btnGerarReceita.addEventListener('click', async () => {
                ...
                const prompt = `Gere 3 ideias de receitas (título, ingredientes e preparo) adequadas para a Fase ${fase} da dieta bariátrica. Priorize alto teor de proteína e facilidade de digestão. Use formatação de lista para facilitar a leitura.`;

                const systemInstruction = "Você é um nutricionista culinário focado em receitas bariátricas. Seu objetivo é fornecer receitas seguras, ricas em proteína e adequadas à fase alimentar do paciente (Líquida, Cremosa, Pastosa, Branda, Sólida).";
            ...
            // --- FUNÇÕES DE NAVEGAÇÃO E GRÁFICOS (ORIGINAIS) ---
